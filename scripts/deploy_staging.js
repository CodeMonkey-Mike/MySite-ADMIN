...
const PORT = 9027;
const SERVED_FOLDER = '/home/dominitech/workspace/mikeneder.me/mikeneder-admin/master';
...
const main = async () => {
  try {
    ...
    // Reload staging site
    await exec('/home/dominitech/.npm-global/bin/pm2 reload admin-mn-staging');
    ...
    const cleanup = async (prNumber) => {
      ...
      await exec(`echo '${SUDO_PASSWORD}' | sudo -S rm -rf /var/www/qa.${DOMAIN}/qa${prNumber}.admin.${DOMAIN}`);
      await exec(`echo '${SUDO_PASSWORD}' | sudo -S rm -rf /home/dominitech/workspace/${DOMAIN}/mikeneder-admin/${prNumber}`);
      await exec(`/home/dominitech/.npm-global/bin/pm2 delete qa${prNumber}.admin.${DOMAIN}`);
      await exec('/home/dominitech/.npm-global/bin/pm2 save');
      await exec(`echo '${SUDO_PASSWORD}' | sudo -S rm /etc/nginx/sites-available/qa${prNumber}.admin.${DOMAIN}`);
      await exec(`echo '${SUDO_PASSWORD}' | sudo -S rm /etc/nginx/sites-enabled/qa${prNumber}.admin.${DOMAIN}`);
